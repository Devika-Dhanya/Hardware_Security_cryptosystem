`timescale 1ns/1ps
// =============================================================================
// Self-Checking Testbench for LTE_PSC_IN_SMA (compatible with V=16/32/64/128)
// - Strict negedge loading (matches your style)
// - Golden negacyclic schoolbook reference: W = B*D mod (x^N + 1) [paper def] [file:1]
// - Online checker for DUT's segment-interleaved output order:
//      per segment: TOP[V] then BOT[V]
// - Watchdog avoids hang; timeout scales with V
// =============================================================================
module tb_proposed_unmasked;
...
reg signed [63:0]   GOLDW[0:N-1];  // wide golden
reg signed [WW-1:0] GOLD [0:N-1];  // truncated golden (matches DUT width)
...
real    comp_time_ns, throughput;

integer errors, out_count, first_seen, run_id;

// Output order tracker (matches your DUT):
// segment-interleaved: top(V) then bot(V), for seg_out = 0..U_SEG-1
reg [8:0]          seg_out;
...
always @(posedge clk) begin
    if (w_valid) begin
        ...
        // TOP half_out=0 => idx = seg*V + lane
        // BOT half_out=1 => idx = N_HALF + seg*V + lane
...
task print_latency_report;
    begin
        ...
        comp_time_ns      = comp_latency * CLK_PERIOD_NS;
        throughput        = (comp_latency > 0) ? (N * 1.0 / comp_latency) : 0.0;

        $display("\n================================================================");
        $display("  LATENCY REPORT  ---  RUN %0d (seed=%0d)  V=%0d", run_id, run_id-1, V);
        $display("================================================================");
        $display("  Clock period              : %0d ns", CLK_PERIOD_NS);
        $display("  LOAD PHASE                : %0d cycles", load_latency);
        $display("  COMPUTATION PHASE         : %0d cycles (%.1f ns)", comp_latency, comp_time_ns);
        $display("  FIRST OUTPUT LATENCY      : %0d cycles", first_out_latency);
        $display("  OUTPUT DURATION           : %0d cycles", output_duration);
        $display("----------------------------------------------------------------");
        $display("  SUMMARY");
        $display("    Total Pipeline Latency  : %0d cycles", total_latency);
        $display("    Outputs received        : %0d / %0d", out_count, N);
        $display("    Throughput              : %.4f outputs/cycle", throughput);
        $display("    Errors                  : %0d", errors);
        $display("    Functional status       : %s", (errors==0 && out_count==N) ? "PASS" : "FAIL");
        $display("================================================================\n");
    end
endtask
...
task run_test;
    ...
    begin
        ...
        // Deterministic patterns (easy to debug)
        for (gi = 0; gi < N; gi = gi + 1) begin
            case ((gi + seed) % 3)
                0:       B[gi] =  1;
                1:       B[gi] = -1;
        ...
        // Watchdog scaled with V (bigger V should finish faster)
        // generous bound: 2000 + 40*(N_HALF/V)*N_HALF
...
initial begin
    // optional VCD (Vivado xsim ignores $dump* sometimes, harmless)
    $dumpfile("tb_LTE_PSC_IN_SMA.vcd");
    $dumpvars(0, tb_LTE_PSC_IN_SMA);
